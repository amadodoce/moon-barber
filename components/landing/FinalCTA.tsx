import Link from "next/link";

export function FinalCTA() {
  return (
    <section className="bg-[#0c0b09] py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
          <div className="max-w-md">
            <h2 className="text-2xl font-bold text-[#f5f0e8] md:text-3xl">
              همین الان نوبت خود را رزرو کنید
            </h2>
            <p className="mt-2 text-sm text-[#6a6458]">
              وقت شما ارزشمند است.
            </p>
          </div>
          <Link
            href="/book"
            className="inline-flex items-center gap-2 rounded-lg bg-[#D4A853] px-7 py-3.5 text-base font-semibold text-[#0c0b09] transition-colors duration-200 hover:bg-[#C49A48]"
          >
            رزرو نوبت
          </Link>
        </div>
      </div>
    </section>
  );
}
